#include <tree_sitter/parser.h>

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#define LANGUAGE_VERSION 9
#define STATE_COUNT 67
#define SYMBOL_COUNT 33
#define ALIAS_COUNT 1
#define TOKEN_COUNT 18
#define EXTERNAL_TOKEN_COUNT 0
#define MAX_ALIAS_SEQUENCE_LENGTH 4

enum {
  anon_sym_POUND_LPAREN = 1,
  anon_sym_RPAREN = 2,
  anon_sym_POUND_SLASH_SLASH = 3,
  aux_sym_comment_token1 = 4,
  anon_sym_POUND_SLASH_STAR = 5,
  aux_sym_comment_token2 = 6,
  sym__tag_symbol = 7,
  sym_identifier = 8,
  sym_string = 9,
  anon_sym_DQUOTE = 10,
  anon_sym_in = 11,
  aux_sym_operator_token1 = 12,
  anon_sym_DOT = 13,
  anon_sym_LPAREN = 14,
  anon_sym_else = 15,
  anon_sym_LBRACE = 16,
  anon_sym_RBRACE = 17,
  sym_source_file = 18,
  sym__definition = 19,
  sym_tag = 20,
  sym__tag_components = 21,
  sym_variable = 22,
  sym_comment = 23,
  sym_string_parameter = 24,
  sym_operator = 25,
  sym_parameter_list = 26,
  sym__parameters = 27,
  sym_else = 28,
  sym_body = 29,
  sym__definitions = 30,
  aux_sym_source_file_repeat1 = 31,
  aux_sym__parameters_repeat1 = 32,
  anon_alias_sym_tag_name = 33,
};

static const char *ts_symbol_names[] = {
  [ts_builtin_sym_end] = "end",
  [anon_sym_POUND_LPAREN] = "#(",
  [anon_sym_RPAREN] = ")",
  [anon_sym_POUND_SLASH_SLASH] = "#//",
  [aux_sym_comment_token1] = "comment_token1",
  [anon_sym_POUND_SLASH_STAR] = "#/*",
  [aux_sym_comment_token2] = "comment_token2",
  [sym__tag_symbol] = "_tag_symbol",
  [sym_identifier] = "identifier",
  [sym_string] = "string",
  [anon_sym_DQUOTE] = "\"",
  [anon_sym_in] = "in",
  [aux_sym_operator_token1] = "operator_token1",
  [anon_sym_DOT] = ".",
  [anon_sym_LPAREN] = "(",
  [anon_sym_else] = "else",
  [anon_sym_LBRACE] = "{",
  [anon_sym_RBRACE] = "}",
  [sym_source_file] = "source_file",
  [sym__definition] = "_definition",
  [sym_tag] = "tag",
  [sym__tag_components] = "_tag_components",
  [sym_variable] = "variable",
  [sym_comment] = "comment",
  [sym_string_parameter] = "string_parameter",
  [sym_operator] = "operator",
  [sym_parameter_list] = "parameter_list",
  [sym__parameters] = "_parameters",
  [sym_else] = "else",
  [sym_body] = "body",
  [sym__definitions] = "_definitions",
  [aux_sym_source_file_repeat1] = "source_file_repeat1",
  [aux_sym__parameters_repeat1] = "_parameters_repeat1",
  [anon_alias_sym_tag_name] = "tag_name",
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
  [ts_builtin_sym_end] = {
    .visible = false,
    .named = true,
  },
  [anon_sym_POUND_LPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_POUND_SLASH_SLASH] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_comment_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_POUND_SLASH_STAR] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_comment_token2] = {
    .visible = false,
    .named = false,
  },
  [sym__tag_symbol] = {
    .visible = false,
    .named = true,
  },
  [sym_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_string] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_DQUOTE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_in] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_operator_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_DOT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_else] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LBRACE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACE] = {
    .visible = true,
    .named = false,
  },
  [sym_source_file] = {
    .visible = true,
    .named = true,
  },
  [sym__definition] = {
    .visible = false,
    .named = true,
  },
  [sym_tag] = {
    .visible = true,
    .named = true,
  },
  [sym__tag_components] = {
    .visible = false,
    .named = true,
  },
  [sym_variable] = {
    .visible = true,
    .named = true,
  },
  [sym_comment] = {
    .visible = true,
    .named = true,
  },
  [sym_string_parameter] = {
    .visible = true,
    .named = true,
  },
  [sym_operator] = {
    .visible = true,
    .named = true,
  },
  [sym_parameter_list] = {
    .visible = true,
    .named = true,
  },
  [sym__parameters] = {
    .visible = false,
    .named = true,
  },
  [sym_else] = {
    .visible = true,
    .named = true,
  },
  [sym_body] = {
    .visible = true,
    .named = true,
  },
  [sym__definitions] = {
    .visible = false,
    .named = true,
  },
  [aux_sym_source_file_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym__parameters_repeat1] = {
    .visible = false,
    .named = false,
  },
  [anon_alias_sym_tag_name] = {
    .visible = true,
    .named = false,
  },
};

static TSSymbol ts_alias_sequences[2][MAX_ALIAS_SEQUENCE_LENGTH] = {
  [1] = {
    [0] = anon_alias_sym_tag_name,
  },
};

static bool ts_lex(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  switch (state) {
    case 0:
      if (lookahead == 0)
        ADVANCE(1);
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      if (lookahead != 0)
        SKIP(0);
      END_STATE();
    case 1:
      ACCEPT_TOKEN(ts_builtin_sym_end);
      END_STATE();
    case 2:
      ACCEPT_TOKEN(sym__tag_symbol);
      if (lookahead == '#')
        ADVANCE(6);
      if (lookahead == '(')
        ADVANCE(7);
      if (lookahead == '/')
        ADVANCE(8);
      END_STATE();
    case 3:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == 'l')
        ADVANCE(9);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      if (lookahead != 0)
        SKIP(10);
      END_STATE();
    case 4:
      ACCEPT_TOKEN(anon_sym_LBRACE);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      END_STATE();
    case 5:
      ACCEPT_TOKEN(anon_sym_RBRACE);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      END_STATE();
    case 6:
      ACCEPT_TOKEN(sym__tag_symbol);
      if (lookahead == '#')
        ADVANCE(6);
      END_STATE();
    case 7:
      ACCEPT_TOKEN(anon_sym_POUND_LPAREN);
      END_STATE();
    case 8:
      if (lookahead == '*')
        ADVANCE(11);
      if (lookahead == '/')
        ADVANCE(12);
      END_STATE();
    case 9:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == 's')
        ADVANCE(13);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      if (lookahead != 0)
        SKIP(10);
      END_STATE();
    case 10:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      if (lookahead != 0)
        SKIP(10);
      END_STATE();
    case 11:
      ACCEPT_TOKEN(anon_sym_POUND_SLASH_STAR);
      END_STATE();
    case 12:
      ACCEPT_TOKEN(anon_sym_POUND_SLASH_SLASH);
      END_STATE();
    case 13:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(14);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      if (lookahead != 0)
        SKIP(10);
      END_STATE();
    case 14:
      ACCEPT_TOKEN(anon_sym_else);
      if (lookahead == 'e')
        ADVANCE(3);
      if (lookahead == 'l')
        ADVANCE(9);
      if (lookahead == '{')
        ADVANCE(4);
      if (lookahead == '}')
        ADVANCE(5);
      END_STATE();
    case 15:
      if (lookahead == 0)
        ADVANCE(1);
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead != 0)
        SKIP(15);
      END_STATE();
    case 16:
      if (lookahead == '*')
        ADVANCE(17);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == '\r' ||
          lookahead == ' ' ||
          lookahead == '\'' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z'))
        ADVANCE(16);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(16);
      END_STATE();
    case 17:
      if (lookahead == '*')
        ADVANCE(17);
      if (lookahead == '/')
        ADVANCE(18);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == '\r' ||
          lookahead == ' ' ||
          lookahead == '\'' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z'))
        ADVANCE(16);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(16);
      END_STATE();
    case 18:
      ACCEPT_TOKEN(aux_sym_comment_token2);
      if (lookahead == '*')
        ADVANCE(17);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == '\r' ||
          lookahead == ' ' ||
          lookahead == '\'' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z'))
        ADVANCE(16);
      END_STATE();
    case 19:
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z'))
        ADVANCE(20);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(19);
      END_STATE();
    case 20:
      ACCEPT_TOKEN(sym_identifier);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z'))
        ADVANCE(20);
      END_STATE();
    case 21:
      ACCEPT_TOKEN(aux_sym_comment_token1);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == '\r' ||
          lookahead == ' ' ||
          lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z'))
        ADVANCE(21);
      END_STATE();
    case 22:
      if (lookahead == '(')
        ADVANCE(23);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(22);
      END_STATE();
    case 23:
      ACCEPT_TOKEN(anon_sym_LPAREN);
      if (lookahead == '(')
        ADVANCE(23);
      END_STATE();
    case 24:
      if (lookahead == ')')
        ADVANCE(25);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(24);
      END_STATE();
    case 25:
      ACCEPT_TOKEN(anon_sym_RPAREN);
      if (lookahead == ')')
        ADVANCE(25);
      END_STATE();
    case 26:
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(26);
      END_STATE();
    case 27:
      ACCEPT_TOKEN(anon_sym_DQUOTE);
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      END_STATE();
    case 28:
      ACCEPT_TOKEN(anon_sym_RPAREN);
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      END_STATE();
    case 29:
      ACCEPT_TOKEN(anon_sym_DOT);
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      END_STATE();
    case 30:
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == 'n')
        ADVANCE(32);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(26);
      END_STATE();
    case 31:
      ACCEPT_TOKEN(aux_sym_operator_token1);
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      END_STATE();
    case 32:
      ACCEPT_TOKEN(anon_sym_in);
      if (lookahead == '"')
        ADVANCE(27);
      if (lookahead == ')')
        ADVANCE(28);
      if (lookahead == '.')
        ADVANCE(29);
      if (lookahead == 'i')
        ADVANCE(30);
      if (lookahead == '!' ||
          lookahead == '%' ||
          lookahead == '&' ||
          lookahead == '*' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '/') ||
          ('<' <= lookahead && lookahead <= '>') ||
          lookahead == '|')
        ADVANCE(31);
      END_STATE();
    case 33:
      if (lookahead == 0)
        ADVANCE(1);
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(34);
      if (lookahead == '{')
        ADVANCE(35);
      if (lookahead != 0)
        SKIP(33);
      END_STATE();
    case 34:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(34);
      if (lookahead == 'l')
        ADVANCE(36);
      if (lookahead == '{')
        ADVANCE(35);
      if (lookahead != 0)
        SKIP(37);
      END_STATE();
    case 35:
      ACCEPT_TOKEN(anon_sym_LBRACE);
      if (lookahead == 'e')
        ADVANCE(34);
      if (lookahead == '{')
        ADVANCE(35);
      END_STATE();
    case 36:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(34);
      if (lookahead == 's')
        ADVANCE(38);
      if (lookahead == '{')
        ADVANCE(35);
      if (lookahead != 0)
        SKIP(37);
      END_STATE();
    case 37:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(34);
      if (lookahead == '{')
        ADVANCE(35);
      if (lookahead != 0)
        SKIP(37);
      END_STATE();
    case 38:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(39);
      if (lookahead == '{')
        ADVANCE(35);
      if (lookahead != 0)
        SKIP(37);
      END_STATE();
    case 39:
      ACCEPT_TOKEN(anon_sym_else);
      if (lookahead == 'e')
        ADVANCE(34);
      if (lookahead == 'l')
        ADVANCE(36);
      if (lookahead == '{')
        ADVANCE(35);
      END_STATE();
    case 40:
      if (lookahead == '#')
        ADVANCE(41);
      if (lookahead == '\n' ||
          lookahead == '"')
        SKIP(40);
      if (lookahead != 0)
        ADVANCE(42);
      END_STATE();
    case 41:
      ACCEPT_TOKEN(sym_string);
      if (lookahead != 0 &&
          lookahead != '\n' &&
          lookahead != '"')
        ADVANCE(41);
      END_STATE();
    case 42:
      ACCEPT_TOKEN(sym_string);
      if (lookahead == '#')
        ADVANCE(41);
      if (lookahead != 0 &&
          lookahead != '\n' &&
          lookahead != '"')
        ADVANCE(42);
      END_STATE();
    case 43:
      if (lookahead == '{')
        ADVANCE(44);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(43);
      END_STATE();
    case 44:
      ACCEPT_TOKEN(anon_sym_LBRACE);
      if (lookahead == '{')
        ADVANCE(44);
      END_STATE();
    case 45:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == '}')
        ADVANCE(46);
      if (lookahead != 0)
        SKIP(45);
      END_STATE();
    case 46:
      ACCEPT_TOKEN(anon_sym_RBRACE);
      if (lookahead == '}')
        ADVANCE(46);
      END_STATE();
    case 47:
      if (lookahead == 0)
        ADVANCE(1);
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(48);
      if (lookahead != 0)
        SKIP(47);
      END_STATE();
    case 48:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(48);
      if (lookahead == 'l')
        ADVANCE(49);
      if (lookahead != 0)
        SKIP(50);
      END_STATE();
    case 49:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(48);
      if (lookahead == 's')
        ADVANCE(51);
      if (lookahead != 0)
        SKIP(50);
      END_STATE();
    case 50:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(48);
      if (lookahead != 0)
        SKIP(50);
      END_STATE();
    case 51:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(52);
      if (lookahead != 0)
        SKIP(50);
      END_STATE();
    case 52:
      ACCEPT_TOKEN(anon_sym_else);
      if (lookahead == 'e')
        ADVANCE(48);
      if (lookahead == 'l')
        ADVANCE(49);
      END_STATE();
    case 53:
      if (lookahead == '"')
        ADVANCE(54);
      if (lookahead != 0 &&
          lookahead != '#')
        SKIP(53);
      END_STATE();
    case 54:
      ACCEPT_TOKEN(anon_sym_DQUOTE);
      if (lookahead == '"')
        ADVANCE(54);
      END_STATE();
    case 55:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(56);
      if (lookahead == '}')
        ADVANCE(57);
      if (lookahead != 0)
        SKIP(55);
      END_STATE();
    case 56:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(56);
      if (lookahead == 'l')
        ADVANCE(58);
      if (lookahead == '}')
        ADVANCE(57);
      if (lookahead != 0)
        SKIP(55);
      END_STATE();
    case 57:
      ACCEPT_TOKEN(anon_sym_RBRACE);
      if (lookahead == 'e')
        ADVANCE(56);
      if (lookahead == '}')
        ADVANCE(57);
      END_STATE();
    case 58:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(56);
      if (lookahead == 's')
        ADVANCE(59);
      if (lookahead == '}')
        ADVANCE(57);
      if (lookahead != 0)
        SKIP(55);
      END_STATE();
    case 59:
      if (lookahead == '#')
        ADVANCE(2);
      if (lookahead == 'e')
        ADVANCE(60);
      if (lookahead == '}')
        ADVANCE(57);
      if (lookahead != 0)
        SKIP(55);
      END_STATE();
    case 60:
      ACCEPT_TOKEN(anon_sym_else);
      if (lookahead == 'e')
        ADVANCE(56);
      if (lookahead == 'l')
        ADVANCE(58);
      if (lookahead == '}')
        ADVANCE(57);
      END_STATE();
    default:
      return false;
  }
}

static TSLexMode ts_lex_modes[STATE_COUNT] = {
  [0] = {.lex_state = 0},
  [1] = {.lex_state = 15},
  [2] = {.lex_state = 16},
  [3] = {.lex_state = 19},
  [4] = {.lex_state = 19},
  [5] = {.lex_state = 21},
  [6] = {.lex_state = 0},
  [7] = {.lex_state = 15},
  [8] = {.lex_state = 15},
  [9] = {.lex_state = 22},
  [10] = {.lex_state = 15},
  [11] = {.lex_state = 24},
  [12] = {.lex_state = 15},
  [13] = {.lex_state = 26},
  [14] = {.lex_state = 33},
  [15] = {.lex_state = 15},
  [16] = {.lex_state = 26},
  [17] = {.lex_state = 40},
  [18] = {.lex_state = 33},
  [19] = {.lex_state = 26},
  [20] = {.lex_state = 24},
  [21] = {.lex_state = 43},
  [22] = {.lex_state = 45},
  [23] = {.lex_state = 15},
  [24] = {.lex_state = 47},
  [25] = {.lex_state = 53},
  [26] = {.lex_state = 26},
  [27] = {.lex_state = 33},
  [28] = {.lex_state = 45},
  [29] = {.lex_state = 15},
  [30] = {.lex_state = 16},
  [31] = {.lex_state = 19},
  [32] = {.lex_state = 19},
  [33] = {.lex_state = 47},
  [34] = {.lex_state = 21},
  [35] = {.lex_state = 45},
  [36] = {.lex_state = 45},
  [37] = {.lex_state = 15},
  [38] = {.lex_state = 26},
  [39] = {.lex_state = 15},
  [40] = {.lex_state = 45},
  [41] = {.lex_state = 45},
  [42] = {.lex_state = 22},
  [43] = {.lex_state = 45},
  [44] = {.lex_state = 24},
  [45] = {.lex_state = 45},
  [46] = {.lex_state = 47},
  [47] = {.lex_state = 15},
  [48] = {.lex_state = 26},
  [49] = {.lex_state = 0},
  [50] = {.lex_state = 45},
  [51] = {.lex_state = 0},
  [52] = {.lex_state = 24},
  [53] = {.lex_state = 43},
  [54] = {.lex_state = 45},
  [55] = {.lex_state = 45},
  [56] = {.lex_state = 55},
  [57] = {.lex_state = 0},
  [58] = {.lex_state = 45},
  [59] = {.lex_state = 45},
  [60] = {.lex_state = 55},
  [61] = {.lex_state = 45},
  [62] = {.lex_state = 45},
  [63] = {.lex_state = 45},
  [64] = {.lex_state = 45},
  [65] = {.lex_state = 55},
  [66] = {.lex_state = 45},
};

static uint16_t ts_parse_table[STATE_COUNT][SYMBOL_COUNT] = {
  [0] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(1),
    [anon_sym_else] = ACTIONS(1),
    [ts_builtin_sym_end] = ACTIONS(1),
    [anon_sym_POUND_LPAREN] = ACTIONS(1),
    [anon_sym_RBRACE] = ACTIONS(1),
    [anon_sym_LBRACE] = ACTIONS(1),
    [sym__tag_symbol] = ACTIONS(1),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(1),
  },
  [1] = {
    [aux_sym_source_file_repeat1] = STATE(7),
    [sym_variable] = STATE(7),
    [sym__definition] = STATE(7),
    [sym_tag] = STATE(7),
    [sym_source_file] = STATE(6),
    [sym_comment] = STATE(7),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(3),
    [ts_builtin_sym_end] = ACTIONS(5),
    [sym__tag_symbol] = ACTIONS(7),
    [anon_sym_POUND_LPAREN] = ACTIONS(9),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(11),
  },
  [2] = {
    [aux_sym_comment_token2] = ACTIONS(13),
  },
  [3] = {
    [sym__tag_components] = STATE(10),
    [sym_identifier] = ACTIONS(15),
  },
  [4] = {
    [sym_identifier] = ACTIONS(17),
  },
  [5] = {
    [aux_sym_comment_token1] = ACTIONS(13),
  },
  [6] = {
    [ts_builtin_sym_end] = ACTIONS(19),
  },
  [7] = {
    [sym_comment] = STATE(12),
    [sym_variable] = STATE(12),
    [sym__definition] = STATE(12),
    [sym_tag] = STATE(12),
    [aux_sym_source_file_repeat1] = STATE(12),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(3),
    [ts_builtin_sym_end] = ACTIONS(21),
    [sym__tag_symbol] = ACTIONS(7),
    [anon_sym_POUND_LPAREN] = ACTIONS(9),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(11),
  },
  [8] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(23),
    [sym__tag_symbol] = ACTIONS(25),
    [ts_builtin_sym_end] = ACTIONS(23),
    [anon_sym_POUND_LPAREN] = ACTIONS(23),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(23),
  },
  [9] = {
    [sym_parameter_list] = STATE(14),
    [anon_sym_LPAREN] = ACTIONS(27),
  },
  [10] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(29),
    [sym__tag_symbol] = ACTIONS(31),
    [ts_builtin_sym_end] = ACTIONS(29),
    [anon_sym_POUND_LPAREN] = ACTIONS(29),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(29),
  },
  [11] = {
    [anon_sym_RPAREN] = ACTIONS(33),
  },
  [12] = {
    [sym_comment] = STATE(12),
    [sym_variable] = STATE(12),
    [sym__definition] = STATE(12),
    [sym_tag] = STATE(12),
    [aux_sym_source_file_repeat1] = STATE(12),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(35),
    [sym__tag_symbol] = ACTIONS(38),
    [ts_builtin_sym_end] = ACTIONS(41),
    [anon_sym_POUND_LPAREN] = ACTIONS(43),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(46),
  },
  [13] = {
    [aux_sym__parameters_repeat1] = STATE(19),
    [sym_string_parameter] = STATE(19),
    [sym__parameters] = STATE(20),
    [sym_operator] = STATE(19),
    [anon_sym_DOT] = ACTIONS(49),
    [anon_sym_DQUOTE] = ACTIONS(51),
    [anon_sym_RPAREN] = ACTIONS(53),
    [aux_sym_operator_token1] = ACTIONS(49),
    [anon_sym_in] = ACTIONS(49),
  },
  [14] = {
    [sym_else] = STATE(23),
    [sym_body] = STATE(24),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(55),
    [sym__tag_symbol] = ACTIONS(55),
    [ts_builtin_sym_end] = ACTIONS(57),
    [anon_sym_POUND_LPAREN] = ACTIONS(55),
    [anon_sym_else] = ACTIONS(59),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(55),
    [anon_sym_LBRACE] = ACTIONS(61),
  },
  [15] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(63),
    [sym__tag_symbol] = ACTIONS(65),
    [ts_builtin_sym_end] = ACTIONS(63),
    [anon_sym_POUND_LPAREN] = ACTIONS(63),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(63),
  },
  [16] = {
    [anon_sym_DOT] = ACTIONS(67),
    [anon_sym_RPAREN] = ACTIONS(67),
    [anon_sym_DQUOTE] = ACTIONS(67),
    [aux_sym_operator_token1] = ACTIONS(67),
    [anon_sym_in] = ACTIONS(67),
  },
  [17] = {
    [sym_string] = ACTIONS(69),
  },
  [18] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(71),
    [sym__tag_symbol] = ACTIONS(71),
    [anon_sym_else] = ACTIONS(73),
    [anon_sym_POUND_LPAREN] = ACTIONS(71),
    [ts_builtin_sym_end] = ACTIONS(73),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(71),
    [anon_sym_LBRACE] = ACTIONS(73),
  },
  [19] = {
    [sym_string_parameter] = STATE(26),
    [sym_operator] = STATE(26),
    [aux_sym__parameters_repeat1] = STATE(26),
    [anon_sym_DOT] = ACTIONS(49),
    [anon_sym_RPAREN] = ACTIONS(75),
    [anon_sym_DQUOTE] = ACTIONS(51),
    [aux_sym_operator_token1] = ACTIONS(49),
    [anon_sym_in] = ACTIONS(49),
  },
  [20] = {
    [anon_sym_RPAREN] = ACTIONS(77),
  },
  [21] = {
    [sym_body] = STATE(29),
    [anon_sym_LBRACE] = ACTIONS(79),
  },
  [22] = {
    [sym_comment] = STATE(35),
    [sym_variable] = STATE(35),
    [sym__definition] = STATE(35),
    [sym_tag] = STATE(35),
    [sym__definitions] = STATE(36),
    [aux_sym_source_file_repeat1] = STATE(35),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(81),
    [sym__tag_symbol] = ACTIONS(83),
    [anon_sym_POUND_LPAREN] = ACTIONS(85),
    [anon_sym_RBRACE] = ACTIONS(87),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(89),
  },
  [23] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(91),
    [sym__tag_symbol] = ACTIONS(93),
    [ts_builtin_sym_end] = ACTIONS(91),
    [anon_sym_POUND_LPAREN] = ACTIONS(91),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(91),
  },
  [24] = {
    [sym_else] = STATE(37),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(93),
    [sym__tag_symbol] = ACTIONS(93),
    [ts_builtin_sym_end] = ACTIONS(91),
    [anon_sym_POUND_LPAREN] = ACTIONS(93),
    [anon_sym_else] = ACTIONS(59),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(93),
  },
  [25] = {
    [anon_sym_DQUOTE] = ACTIONS(95),
  },
  [26] = {
    [sym_string_parameter] = STATE(26),
    [sym_operator] = STATE(26),
    [aux_sym__parameters_repeat1] = STATE(26),
    [anon_sym_DOT] = ACTIONS(97),
    [anon_sym_RPAREN] = ACTIONS(100),
    [anon_sym_DQUOTE] = ACTIONS(102),
    [aux_sym_operator_token1] = ACTIONS(97),
    [anon_sym_in] = ACTIONS(97),
  },
  [27] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(105),
    [sym__tag_symbol] = ACTIONS(105),
    [anon_sym_else] = ACTIONS(107),
    [anon_sym_POUND_LPAREN] = ACTIONS(105),
    [ts_builtin_sym_end] = ACTIONS(107),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(105),
    [anon_sym_LBRACE] = ACTIONS(107),
  },
  [28] = {
    [sym_comment] = STATE(35),
    [sym_variable] = STATE(35),
    [sym__definition] = STATE(35),
    [sym_tag] = STATE(35),
    [sym__definitions] = STATE(40),
    [aux_sym_source_file_repeat1] = STATE(35),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(81),
    [sym__tag_symbol] = ACTIONS(83),
    [anon_sym_POUND_LPAREN] = ACTIONS(85),
    [anon_sym_RBRACE] = ACTIONS(109),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(89),
  },
  [29] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(111),
    [sym__tag_symbol] = ACTIONS(113),
    [ts_builtin_sym_end] = ACTIONS(111),
    [anon_sym_POUND_LPAREN] = ACTIONS(111),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(111),
  },
  [30] = {
    [aux_sym_comment_token2] = ACTIONS(115),
  },
  [31] = {
    [sym__tag_components] = STATE(43),
    [sym_identifier] = ACTIONS(117),
  },
  [32] = {
    [sym_identifier] = ACTIONS(119),
  },
  [33] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(121),
    [sym__tag_symbol] = ACTIONS(121),
    [anon_sym_else] = ACTIONS(123),
    [anon_sym_POUND_LPAREN] = ACTIONS(121),
    [ts_builtin_sym_end] = ACTIONS(123),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(121),
  },
  [34] = {
    [aux_sym_comment_token1] = ACTIONS(115),
  },
  [35] = {
    [sym_comment] = STATE(45),
    [sym_variable] = STATE(45),
    [sym__definition] = STATE(45),
    [sym_tag] = STATE(45),
    [aux_sym_source_file_repeat1] = STATE(45),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(81),
    [sym__tag_symbol] = ACTIONS(83),
    [anon_sym_RBRACE] = ACTIONS(125),
    [anon_sym_POUND_LPAREN] = ACTIONS(85),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(89),
  },
  [36] = {
    [anon_sym_RBRACE] = ACTIONS(127),
  },
  [37] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(129),
    [sym__tag_symbol] = ACTIONS(131),
    [ts_builtin_sym_end] = ACTIONS(129),
    [anon_sym_POUND_LPAREN] = ACTIONS(129),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(129),
  },
  [38] = {
    [anon_sym_DOT] = ACTIONS(133),
    [anon_sym_RPAREN] = ACTIONS(133),
    [anon_sym_DQUOTE] = ACTIONS(133),
    [aux_sym_operator_token1] = ACTIONS(133),
    [anon_sym_in] = ACTIONS(133),
  },
  [39] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(123),
    [sym__tag_symbol] = ACTIONS(121),
    [ts_builtin_sym_end] = ACTIONS(123),
    [anon_sym_POUND_LPAREN] = ACTIONS(123),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(123),
  },
  [40] = {
    [anon_sym_RBRACE] = ACTIONS(135),
  },
  [41] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(25),
    [sym__tag_symbol] = ACTIONS(25),
    [anon_sym_POUND_LPAREN] = ACTIONS(25),
    [anon_sym_RBRACE] = ACTIONS(23),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(25),
  },
  [42] = {
    [sym_parameter_list] = STATE(49),
    [anon_sym_LPAREN] = ACTIONS(137),
  },
  [43] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(31),
    [sym__tag_symbol] = ACTIONS(31),
    [anon_sym_POUND_LPAREN] = ACTIONS(31),
    [anon_sym_RBRACE] = ACTIONS(29),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(31),
  },
  [44] = {
    [anon_sym_RPAREN] = ACTIONS(139),
  },
  [45] = {
    [sym_comment] = STATE(45),
    [sym_variable] = STATE(45),
    [sym__definition] = STATE(45),
    [sym_tag] = STATE(45),
    [aux_sym_source_file_repeat1] = STATE(45),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(141),
    [sym__tag_symbol] = ACTIONS(144),
    [anon_sym_POUND_LPAREN] = ACTIONS(147),
    [anon_sym_RBRACE] = ACTIONS(41),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(150),
  },
  [46] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(153),
    [sym__tag_symbol] = ACTIONS(153),
    [anon_sym_else] = ACTIONS(155),
    [anon_sym_POUND_LPAREN] = ACTIONS(153),
    [ts_builtin_sym_end] = ACTIONS(155),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(153),
  },
  [47] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(155),
    [sym__tag_symbol] = ACTIONS(153),
    [ts_builtin_sym_end] = ACTIONS(155),
    [anon_sym_POUND_LPAREN] = ACTIONS(155),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(155),
  },
  [48] = {
    [aux_sym__parameters_repeat1] = STATE(19),
    [sym_string_parameter] = STATE(19),
    [sym__parameters] = STATE(52),
    [sym_operator] = STATE(19),
    [anon_sym_DOT] = ACTIONS(49),
    [anon_sym_DQUOTE] = ACTIONS(51),
    [anon_sym_RPAREN] = ACTIONS(157),
    [aux_sym_operator_token1] = ACTIONS(49),
    [anon_sym_in] = ACTIONS(49),
  },
  [49] = {
    [sym_else] = STATE(55),
    [sym_body] = STATE(56),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(55),
    [sym__tag_symbol] = ACTIONS(55),
    [anon_sym_else] = ACTIONS(159),
    [anon_sym_POUND_LPAREN] = ACTIONS(55),
    [anon_sym_RBRACE] = ACTIONS(57),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(55),
    [anon_sym_LBRACE] = ACTIONS(161),
  },
  [50] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(65),
    [sym__tag_symbol] = ACTIONS(65),
    [anon_sym_POUND_LPAREN] = ACTIONS(65),
    [anon_sym_RBRACE] = ACTIONS(63),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(65),
  },
  [51] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(71),
    [sym__tag_symbol] = ACTIONS(71),
    [anon_sym_else] = ACTIONS(73),
    [anon_sym_POUND_LPAREN] = ACTIONS(71),
    [anon_sym_RBRACE] = ACTIONS(73),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(71),
    [anon_sym_LBRACE] = ACTIONS(73),
  },
  [52] = {
    [anon_sym_RPAREN] = ACTIONS(163),
  },
  [53] = {
    [sym_body] = STATE(59),
    [anon_sym_LBRACE] = ACTIONS(165),
  },
  [54] = {
    [sym_comment] = STATE(35),
    [sym_variable] = STATE(35),
    [sym__definition] = STATE(35),
    [sym_tag] = STATE(35),
    [sym__definitions] = STATE(61),
    [aux_sym_source_file_repeat1] = STATE(35),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(81),
    [sym__tag_symbol] = ACTIONS(83),
    [anon_sym_POUND_LPAREN] = ACTIONS(85),
    [anon_sym_RBRACE] = ACTIONS(167),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(89),
  },
  [55] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(93),
    [sym__tag_symbol] = ACTIONS(93),
    [anon_sym_POUND_LPAREN] = ACTIONS(93),
    [anon_sym_RBRACE] = ACTIONS(91),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(93),
  },
  [56] = {
    [sym_else] = STATE(62),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(93),
    [sym__tag_symbol] = ACTIONS(93),
    [anon_sym_else] = ACTIONS(159),
    [anon_sym_POUND_LPAREN] = ACTIONS(93),
    [anon_sym_RBRACE] = ACTIONS(91),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(93),
  },
  [57] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(105),
    [sym__tag_symbol] = ACTIONS(105),
    [anon_sym_else] = ACTIONS(107),
    [anon_sym_POUND_LPAREN] = ACTIONS(105),
    [anon_sym_RBRACE] = ACTIONS(107),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(105),
    [anon_sym_LBRACE] = ACTIONS(107),
  },
  [58] = {
    [sym_comment] = STATE(35),
    [sym_variable] = STATE(35),
    [sym__definition] = STATE(35),
    [sym_tag] = STATE(35),
    [sym__definitions] = STATE(64),
    [aux_sym_source_file_repeat1] = STATE(35),
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(81),
    [sym__tag_symbol] = ACTIONS(83),
    [anon_sym_POUND_LPAREN] = ACTIONS(85),
    [anon_sym_RBRACE] = ACTIONS(169),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(89),
  },
  [59] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(113),
    [sym__tag_symbol] = ACTIONS(113),
    [anon_sym_POUND_LPAREN] = ACTIONS(113),
    [anon_sym_RBRACE] = ACTIONS(111),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(113),
  },
  [60] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(121),
    [sym__tag_symbol] = ACTIONS(121),
    [anon_sym_else] = ACTIONS(123),
    [anon_sym_POUND_LPAREN] = ACTIONS(121),
    [anon_sym_RBRACE] = ACTIONS(123),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(121),
  },
  [61] = {
    [anon_sym_RBRACE] = ACTIONS(171),
  },
  [62] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(131),
    [sym__tag_symbol] = ACTIONS(131),
    [anon_sym_POUND_LPAREN] = ACTIONS(131),
    [anon_sym_RBRACE] = ACTIONS(129),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(131),
  },
  [63] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(121),
    [sym__tag_symbol] = ACTIONS(121),
    [anon_sym_POUND_LPAREN] = ACTIONS(121),
    [anon_sym_RBRACE] = ACTIONS(123),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(121),
  },
  [64] = {
    [anon_sym_RBRACE] = ACTIONS(173),
  },
  [65] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(153),
    [sym__tag_symbol] = ACTIONS(153),
    [anon_sym_else] = ACTIONS(155),
    [anon_sym_POUND_LPAREN] = ACTIONS(153),
    [anon_sym_RBRACE] = ACTIONS(155),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(153),
  },
  [66] = {
    [anon_sym_POUND_SLASH_STAR] = ACTIONS(153),
    [sym__tag_symbol] = ACTIONS(153),
    [anon_sym_POUND_LPAREN] = ACTIONS(153),
    [anon_sym_RBRACE] = ACTIONS(155),
    [anon_sym_POUND_SLASH_SLASH] = ACTIONS(153),
  },
};

static TSParseActionEntry ts_parse_actions[] = {
  [0] = {.count = 0, .reusable = false},
  [1] = {.count = 1, .reusable = false}, RECOVER(),
  [3] = {.count = 1, .reusable = true}, SHIFT(2),
  [5] = {.count = 1, .reusable = true}, REDUCE(sym_source_file, 0),
  [7] = {.count = 1, .reusable = false}, SHIFT(3),
  [9] = {.count = 1, .reusable = true}, SHIFT(4),
  [11] = {.count = 1, .reusable = true}, SHIFT(5),
  [13] = {.count = 1, .reusable = true}, SHIFT(8),
  [15] = {.count = 1, .reusable = true}, SHIFT(9),
  [17] = {.count = 1, .reusable = true}, SHIFT(11),
  [19] = {.count = 1, .reusable = true},  ACCEPT_INPUT(),
  [21] = {.count = 1, .reusable = true}, REDUCE(sym_source_file, 1),
  [23] = {.count = 1, .reusable = true}, REDUCE(sym_comment, 2),
  [25] = {.count = 1, .reusable = false}, REDUCE(sym_comment, 2),
  [27] = {.count = 1, .reusable = true}, SHIFT(13),
  [29] = {.count = 1, .reusable = true}, REDUCE(sym_tag, 2),
  [31] = {.count = 1, .reusable = false}, REDUCE(sym_tag, 2),
  [33] = {.count = 1, .reusable = true}, SHIFT(15),
  [35] = {.count = 2, .reusable = true}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(2),
  [38] = {.count = 2, .reusable = false}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(3),
  [41] = {.count = 1, .reusable = true}, REDUCE(aux_sym_source_file_repeat1, 2),
  [43] = {.count = 2, .reusable = true}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(4),
  [46] = {.count = 2, .reusable = true}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(5),
  [49] = {.count = 1, .reusable = true}, SHIFT(16),
  [51] = {.count = 1, .reusable = true}, SHIFT(17),
  [53] = {.count = 1, .reusable = true}, SHIFT(18),
  [55] = {.count = 1, .reusable = false}, REDUCE(sym__tag_components, 2, .alias_sequence_id = 1),
  [57] = {.count = 1, .reusable = true}, REDUCE(sym__tag_components, 2, .alias_sequence_id = 1),
  [59] = {.count = 1, .reusable = true}, SHIFT(21),
  [61] = {.count = 1, .reusable = true}, SHIFT(22),
  [63] = {.count = 1, .reusable = true}, REDUCE(sym_variable, 3),
  [65] = {.count = 1, .reusable = false}, REDUCE(sym_variable, 3),
  [67] = {.count = 1, .reusable = true}, REDUCE(sym_operator, 1),
  [69] = {.count = 1, .reusable = true}, SHIFT(25),
  [71] = {.count = 1, .reusable = false}, REDUCE(sym_parameter_list, 2),
  [73] = {.count = 1, .reusable = true}, REDUCE(sym_parameter_list, 2),
  [75] = {.count = 1, .reusable = true}, REDUCE(sym__parameters, 1),
  [77] = {.count = 1, .reusable = true}, SHIFT(27),
  [79] = {.count = 1, .reusable = true}, SHIFT(28),
  [81] = {.count = 1, .reusable = false}, SHIFT(30),
  [83] = {.count = 1, .reusable = false}, SHIFT(31),
  [85] = {.count = 1, .reusable = false}, SHIFT(32),
  [87] = {.count = 1, .reusable = true}, SHIFT(33),
  [89] = {.count = 1, .reusable = false}, SHIFT(34),
  [91] = {.count = 1, .reusable = true}, REDUCE(sym__tag_components, 3, .alias_sequence_id = 1),
  [93] = {.count = 1, .reusable = false}, REDUCE(sym__tag_components, 3, .alias_sequence_id = 1),
  [95] = {.count = 1, .reusable = true}, SHIFT(38),
  [97] = {.count = 2, .reusable = true}, REDUCE(aux_sym__parameters_repeat1, 2), SHIFT_REPEAT(16),
  [100] = {.count = 1, .reusable = true}, REDUCE(aux_sym__parameters_repeat1, 2),
  [102] = {.count = 2, .reusable = true}, REDUCE(aux_sym__parameters_repeat1, 2), SHIFT_REPEAT(17),
  [105] = {.count = 1, .reusable = false}, REDUCE(sym_parameter_list, 3),
  [107] = {.count = 1, .reusable = true}, REDUCE(sym_parameter_list, 3),
  [109] = {.count = 1, .reusable = true}, SHIFT(39),
  [111] = {.count = 1, .reusable = true}, REDUCE(sym_else, 2),
  [113] = {.count = 1, .reusable = false}, REDUCE(sym_else, 2),
  [115] = {.count = 1, .reusable = true}, SHIFT(41),
  [117] = {.count = 1, .reusable = true}, SHIFT(42),
  [119] = {.count = 1, .reusable = true}, SHIFT(44),
  [121] = {.count = 1, .reusable = false}, REDUCE(sym_body, 2),
  [123] = {.count = 1, .reusable = true}, REDUCE(sym_body, 2),
  [125] = {.count = 1, .reusable = true}, REDUCE(sym__definitions, 1),
  [127] = {.count = 1, .reusable = true}, SHIFT(46),
  [129] = {.count = 1, .reusable = true}, REDUCE(sym__tag_components, 4, .alias_sequence_id = 1),
  [131] = {.count = 1, .reusable = false}, REDUCE(sym__tag_components, 4, .alias_sequence_id = 1),
  [133] = {.count = 1, .reusable = true}, REDUCE(sym_string_parameter, 3),
  [135] = {.count = 1, .reusable = true}, SHIFT(47),
  [137] = {.count = 1, .reusable = true}, SHIFT(48),
  [139] = {.count = 1, .reusable = true}, SHIFT(50),
  [141] = {.count = 2, .reusable = false}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(30),
  [144] = {.count = 2, .reusable = false}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(31),
  [147] = {.count = 2, .reusable = false}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(32),
  [150] = {.count = 2, .reusable = false}, REDUCE(aux_sym_source_file_repeat1, 2), SHIFT_REPEAT(34),
  [153] = {.count = 1, .reusable = false}, REDUCE(sym_body, 3),
  [155] = {.count = 1, .reusable = true}, REDUCE(sym_body, 3),
  [157] = {.count = 1, .reusable = true}, SHIFT(51),
  [159] = {.count = 1, .reusable = true}, SHIFT(53),
  [161] = {.count = 1, .reusable = true}, SHIFT(54),
  [163] = {.count = 1, .reusable = true}, SHIFT(57),
  [165] = {.count = 1, .reusable = true}, SHIFT(58),
  [167] = {.count = 1, .reusable = true}, SHIFT(60),
  [169] = {.count = 1, .reusable = true}, SHIFT(63),
  [171] = {.count = 1, .reusable = true}, SHIFT(65),
  [173] = {.count = 1, .reusable = true}, SHIFT(66),
};

#ifdef _WIN32
#define extern __declspec(dllexport)
#endif

extern const TSLanguage *tree_sitter_leaf() {
  static TSLanguage language = {
    .version = LANGUAGE_VERSION,
    .symbol_count = SYMBOL_COUNT,
    .alias_count = ALIAS_COUNT,
    .token_count = TOKEN_COUNT,
    .symbol_metadata = ts_symbol_metadata,
    .parse_table = (const unsigned short *)ts_parse_table,
    .parse_actions = ts_parse_actions,
    .lex_modes = ts_lex_modes,
    .symbol_names = ts_symbol_names,
    .alias_sequences = (const TSSymbol *)ts_alias_sequences,
    .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
    .lex_fn = ts_lex,
    .external_token_count = EXTERNAL_TOKEN_COUNT,
  };
  return &language;
}
